...
- Flag syntax must be "flag=arg" ("flag arg" not supported).`,
	...
	cmd.Flags().UintVar(virtComponents[allComponent], allComponent, NoFlag, "show/set all component log verbosity (0-9)")
	cmd.Flags().Lookup(allComponent).NoOptDefVal = strconv.FormatUint(noArg, 10)
	...
	cmd.MarkFlagsMutuallyExclusive("reset", allComponent)
...
func createOutputLines(verbosityVal map[string]uint) []string {
	...
	allIsSet := (*virtComponents[allComponent] != NoFlag)
...
func setVerbosity(currentLv map[string]uint, patchData *[]patch.PatchOperation) {
	...
	if *virtComponents[allComponent] != NoFlag {
		for componentName := range virtComponents {
			if componentName == allComponent {
				continue
			}
			JSONName := getJSONNameByComponentName(componentName)
			currentLv[JSONName] = *virtComponents[allComponent]
		}
	}
