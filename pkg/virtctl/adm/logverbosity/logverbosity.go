...
const (
	...
	// They should be hidden in the help menu, but there is no option to hide the default value from the help menu.
	...
	NoFlag = 11 // Default value if no flag is specified (dummy, we use cmd.Flags().Changed() to check if a flag is specified)
)
